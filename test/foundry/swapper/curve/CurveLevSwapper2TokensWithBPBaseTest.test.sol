...
import { CurveRemovalType, SwapType, BaseLevSwapper, MockCurveLevSwapper2TokensWithBP, Swapper, IUniswapV3Router, IAngleRouterSidechain } from "borrow-staked/mock/MockCurveLevSwapper2TokensWithBP.sol";
...
contract CurveLevSwapper2TokensWithBPBaseTest is BaseTest {
    ...
    IERC20[3] internal listBPTokens;
    ...
    function setUp() public virtual override {
        ...
        listTokens = [_LUSD, _BPToken];
        listBPTokens = [_DAI, _USDC, _USDT];
    ...
    function testNoDepositDeleverageCollatAndOneCoinToken(uint256 amount, uint256 propToRemove) public {
    ...
    function testDeleverageOneCoinBPToken(
        uint256[4] memory amounts,
        uint256 swapAmount,
        uint256 coinIndex,
        int128 coinSwap
        ...
        IERC20 outToken = listBPTokens[coinIndex];
        uint256 minOneCoin = _deleverageFullyOneCoinFromBP(int128(uint128(coinIndex)), outToken);

        for (uint256 i; i < listTokens.length; i++) {
            assertEq(listTokens[i].balanceOf(_alice), 0);
        }
        for (uint256 i; i < listBPTokens.length; i++) {
            if (i == coinIndex) assertGe(listBPTokens[i].balanceOf(_alice), minOneCoin);
            else assertEq(listBPTokens[i].balanceOf(_alice), 0);
        }
        _assertCommonDeleverage();
    }

    function testDeleverageBalanceBPToken(
        uint256[4] memory amounts,
        uint256 swapAmount,
        uint256 coinIndex,
        int128 coinSwap
        ...
        IERC20 outToken = listBPTokens[coinIndex];
        uint256[3] memory minAmounts = _deleverageFullyBalanceFromBP(int128(uint128(coinIndex)), outToken);

        for (uint256 i; i < listTokens.length; i++) assertEq(listTokens[i].balanceOf(_alice), 0);
        for (uint256 i; i < listBPTokens.length; i++) assertGe(listBPTokens[i].balanceOf(_alice), minAmounts[i]);

        _assertCommonDeleverage();
    }

    function testDeleverageImBalanceBPToken(
        uint256[4] memory amounts,
        uint256 swapAmount,
        uint256 coinIndex,
        int128 coinSwap,
        ...
        IERC20 outToken = listBPTokens[coinIndex];
        (uint256[3] memory amountOut, uint256 keptLPToken) = _deleverageFullyImbalanceFromBP(
            coinIndex,
            outToken,
        ...
        for (uint256 i; i < listTokens.length; i++) {
            if (swapper.indexBPToken() == i) assertGe(listTokens[i].balanceOf(_alice), keptLPToken);
            else assertEq(listTokens[i].balanceOf(_alice), 0);
        }
        for (uint256 i; i < listBPTokens.length; i++) assertEq(listBPTokens[i].balanceOf(_alice), amountOut[i]);
        _assertCommonDeleverage();
    }

    function testDeleverageBalance(uint256[4] memory amounts, uint256 swapAmount, int128 coinSwap) public {
        _depositSwapAndAddLiquidity(amounts);
        _swapToImbalance(coinSwap, coinSwap, swapAmount);
    ...
    function _depositSwapAndAddLiquidity(uint256[4] memory amounts) internal returns (uint256 minAmountOut) {
        // DAI - USDC - USDT - LUSD
        ...
        deal(address(_LUSD), address(_alice), amounts[3]);
    ...
    function _deleverageFullyOneCoinFromBP(int128 coinIndex, IERC20 outToken) internal returns (uint256) {
        vm.startPrank(_alice);
        // deleverage
        uint256 amount = staker.balanceOf(_alice);
        uint256 minOneCoin;
        bytes memory data;
        {
            bytes[] memory oneInchData = new bytes[](0);
            IERC20[] memory sweepTokens = new IERC20[](0);
            uint256 minBP = (_METAPOOL.calc_withdraw_one_coin(amount, 1) * SLIPPAGE_BPS) / _BPS;
            minOneCoin = (_BPPOOL.calc_withdraw_one_coin(minBP, coinIndex) * SLIPPAGE_BPS) / _BPS;
            bytes memory removeBPData = abi.encode(CurveRemovalType.oneCoin, abi.encode(coinIndex, minOneCoin));
            bytes memory removeData = abi.encode(CurveRemovalType.oneCoin, true, abi.encode(1, minBP, removeBPData));
            bytes memory swapData = abi.encode(amount, amount, sweepTokens, oneInchData, removeData);
            bytes memory leverageData = abi.encode(false, _alice, swapData);
            data = abi.encode(address(0), minOneCoin, SwapType.Leverage, leverageData);
        }
    ...
    function _deleverageBalance() internal returns (uint256[2] memory minAmounts) {
        ...
        bytes memory data;
        {
            ...
            bytes memory removeBPData;
            bytes memory removeData = abi.encode(CurveRemovalType.balance, false, abi.encode(minAmounts, removeBPData));
            bytes memory swapData = abi.encode(amount, amount, sweepTokens, oneInchData, removeData);
            bytes memory leverageData = abi.encode(false, _alice, swapData);
    ...
    function _deleverageFullyBalanceFromBP(
        int128 coinIndex,
        IERC20 outToken
    ) internal returns (uint256[3] memory minAmounts) {
        vm.startPrank(_alice);
        // deleverage
        uint256 amount = staker.balanceOf(_alice);
        uint256 minOneCoin;
        ...
        {
            bytes[] memory oneInchData = new bytes[](0);
            IERC20[] memory sweepTokens = new IERC20[](listBPTokens.length - 1);
            {
                uint256 count;
                for (uint256 i; i < listBPTokens.length; i++) {
                    if (i != uint256(uint128(coinIndex))) {
                        sweepTokens[count] = listBPTokens[i];
                        count += 1;
                    }
                }
            }
            ...
            uint256 minBP = (_METAPOOL.calc_withdraw_one_coin(amount, 1) * SLIPPAGE_BPS) / _BPS;
            minAmounts = [
                (_BPPOOL.balances(0) * minBP * SLIPPAGE_BPS) / (_BPS * _BPToken.totalSupply()),
                (_BPPOOL.balances(1) * minBP * SLIPPAGE_BPS) / (_BPS * _BPToken.totalSupply()),
                (_BPPOOL.balances(2) * minBP * SLIPPAGE_BPS) / (_BPS * _BPToken.totalSupply())
            ];
            bytes memory removeBPData = abi.encode(CurveRemovalType.balance, abi.encode(minAmounts));
            bytes memory removeData = abi.encode(CurveRemovalType.oneCoin, true, abi.encode(1, minBP, removeBPData));
            bytes memory swapData = abi.encode(amount, amount, sweepTokens, oneInchData, removeData);
            bytes memory leverageData = abi.encode(false, _alice, swapData);
            data = abi.encode(address(0), minOneCoin, SwapType.Leverage, leverageData);
    ...
    function _deleverageFullyImbalanceFromBP(
        uint256 coinIndex,
        IERC20 outToken,
        uint256 proportionWithdrawToken1,
        uint256 proportionWithdrawToken2
    ) internal returns (uint256[3] memory amountOuts, uint256) {
        vm.startPrank(_alice);
        // deleverage
        uint256 amount = staker.balanceOf(_alice);
        ...
        {
            bytes[] memory oneInchData = new bytes[](0);
            IERC20[] memory sweepTokens = new IERC20[](listBPTokens.length);
            {
                uint256 count;
                for (uint256 i; i < listBPTokens.length; i++) {
                    if (i != coinIndex) {
                ...
                sweepTokens[count] = _BPToken;
            }

            uint256 minBP = (_METAPOOL.calc_withdraw_one_coin(amount, 1) * SLIPPAGE_BPS) / _BPS;
            uint256[3] memory minAmounts = [
                (_BPPOOL.balances(0) * minBP) / (_BPToken.totalSupply()),
                (_BPPOOL.balances(1) * minBP) / (_BPToken.totalSupply()),
                (_BPPOOL.balances(2) * minBP) / (_BPToken.totalSupply())
            ];
            ...
            {
                uint256 curveBalanceDAI = _BPPOOL.balances(0);
                uint256 curveBalanceUSDC = _BPPOOL.balances(1);
                uint256 curveBalanceUSDT = _BPPOOL.balances(2);
            ...
            maxBurnAmount = IMetaPool3(address(_BPPOOL)).calc_token_amount(amountOuts, false);
            maxBurnAmount = minBP - maxBurnAmount;

            bytes memory leverageData;
            {
                bytes memory removeBPData = abi.encode(CurveRemovalType.imbalance, abi.encode(amountOuts));
                bytes memory removeData = abi.encode(
                    CurveRemovalType.oneCoin,
                    true,
                    abi.encode(1, minBP, removeBPData)
                );
                bytes memory swapData = abi.encode(amount, amount, sweepTokens, oneInchData, removeData);
                leverageData = abi.encode(false, _alice, swapData);
            }
            data = abi.encode(address(0), amountOuts[coinIndex], SwapType.Leverage, leverageData);
        }
        staker.transfer(address(swapper), amount);
        swapper.swap(IERC20(address(staker)), outToken, _alice, 0, amount, data);
        ...
        return (amountOuts, maxBurnAmount);
    }
