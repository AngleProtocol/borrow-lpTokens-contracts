...
contract VaultManagerListing is VaultManager {
    ...
    function _beforeTokenTransfer(
        ...
        // If it is burn we also need to checkpoint as the burn didn't trigger yet a change in collateral amount
        if (from != address(0)) {
    ...
    /// @dev Checkpoints the `collateral` of the contract after an update of the
